...
- **Batched release (this branch, THE flat-demand lever)**: `run_headless(batch_release=True)` / GUI "Batched release (1 stop/side)" — all of an order's same-side lines go to ONE station (rotated by order id), cutting stations/order ~7 → ~2.9. **Pair it with `picker_management=True`** (side-wide walks need auto-staffing; alone it's ≈ neutral). Winning stack mgmt+extra+batch: 55.8 o/hr / ~1220 picks/hr at default pillars, 61.8 / ~1390 at the swept optimum `OPTIMAL_HIGHWAY=(25, 68)` (2026-07-29 re-sweep; results/batch_release_ab.md + results/highway_sweep.md). Result field `avg_stations_per_order` tracks the mechanism.
